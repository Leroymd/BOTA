...
const fs = require('fs');
const { TradingBot } = require('./trading-bot'); // Исправлено: импорт из нового файла
...
app.post('/api/bot/start', (req, res) => {
  try {
    ...
    // Настраиваем дополнительные обработчики для позиций
    bot.positionManager.on('position_opened', (position) => {
      io.emit('position_opened', position);
    });
    
    bot.positionManager.on('position_closed', (data) => {
      io.emit('position_closed', data);
    });
    ...
    bot.positionManager.on('position_history_updated', (position) => {
      io.emit('position_history_updated', position);
    });
...
// Мультиботовая система - опциональная часть, если нужно сохранить поддержку
try {
  // Проверяем, существуют ли необходимые файлы
  if (fs.existsSync(path.join(__dirname, 'multi-bot-manager.js')) && 
      fs.existsSync(path.join(__dirname, 'pair-scanner.js'))) {
    ...
    console.log("Мультиботовая система успешно инициализирована.");
    
  } else {
    console.log("Файлы мультиботовой системы не найдены. Запущен только режим одиночного бота.");
  }
} catch (error) {
  console.error("Ошибка при инициализации мультиботовой системы:", error);
  console.log("Запущен только режим одиночного бота.");
}
